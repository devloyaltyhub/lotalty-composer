...
class ClientCreationWizard {
  ...
  async collectClientInfo() {
    ...
    if (availableBusinessTypes.length === 0) {
      logger.error(
        "No business types found! Please create at least one business type first.",
      );
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        name: "loversName",
        message:
          'Nome dos "lovers" da loja (ex: "Na Redeiros", "Biriteiros", "Cafeinados"):',
        validate: (input) =>
          input.trim().length > 0 || "Lovers name is required",
      },
      {
        type: "input",
        name: "adminEmail",
        ...
        default: (answers) => {
          ...
          const normalizedCode = answers.clientCode
            .toLowerCase()
            .replace(/[^a-z0-9-]/g, "");
    ...
    if (answers.firebaseProjectId.length > 30) {
      ...
      logger.error(
        `Please use a shorter client code (max ${30 - 9} characters)`,
      );
      process.exit(1);
    }
  ...
  async createFirebaseProject() {
    ...
    await telegram.clientCreationStarted(
    ...
    await telegram.firebaseProjectCreated(
      this.config.clientName,
  ...
  async deployFirestoreRules() {
    ...
    const rulesPath = path.join(
      __dirname,
      "../../shared/templates/firestore.rules",
    );
    const tempRulesPath = path.join(
    ...
    // Deploy rules
    await firebaseClient.deployFirestoreRules(
      this.config.firebaseProjectId,
      tempRulesPath,
    );
  ...
  generateColorPalette(primaryColor) {
    ...
    const lighten = (value) =>
      Math.min(255, Math.floor(value + (255 - value) * 0.3));
    const primaryLight =
  ...
  saveLocalConfig() {
    ...
    const configData = {
      ...
      companyHint: this.getCompanyHintFromBusinessType(
        this.config.businessType,
      ),
      businessType: this.config.businessType,
      ...
      metadata: {
        ...
        supportUrl:
          this.config.supportUrl || "https://www.loyaltyhub.club/contact",
        privacyUrl:
          this.config.privacyUrl || "https://www.loyaltyhub.club/legal#privacy",
...
`;
  ...
  copyBusinessTypeAssets() {
    ...
    const businessTypeImagesDir = path.join(
      sharedAssetsDir,
      "images",
      this.config.businessType,
    );
    ...
    if (fs.existsSync(businessTypeImagesDir)) {
      ...
      const transparentLogoFile = path.join(
        businessTypeImagesDir,
    ...
    if (!copiedFromBusinessType) {
      logger.warn(
        `Business type ${this.config.businessType} doesn't have logo templates`,
      );
      ...
      if (fs.existsSync(demoAssetsDir)) {
        ...
        const demoTransparentLogo = path.join(
          demoAssetsDir,
          "transparent-logo.png",
        );
  ...
  async generateAndroidKeystore() {
    ...
    try {
      ...
      const result = await androidSetup.setupCredentials(
        this.config.clientCode,
      );
  ...
  async generateAppCheckSetup() {
    ...
    try {
      ...
      logger.info(
        "  ℹ️  Detalhes das ações manuais serão exibidos ao final da execução",
      );
  ...
  async copyCredentialsToCloudService() {
    ...
    if (!serviceAccountPath || !fs.existsSync(serviceAccountPath)) {
      logger.warn(
        "Service account file not found, skipping cloud service credentials copy",
      );
      return;
    }
    ...
    if (!fs.existsSync(cloudServiceCredentialsDir)) {
      ...
      logger.info(
        `Created credentials directory: ${cloudServiceCredentialsDir}`,
      );
    }

    // Destination file: {clientCode}.json
    const destinationPath = path.join(
      cloudServiceCredentialsDir,
      `${this.config.clientCode}.json`,
    );
    ...
    logger.info(
      "Cloud Service will auto-detect this credential on next restart",
    );
  }
  ...
  async createShorebirdConfig() {
    ...
    const clientShorebirdPath = path.join(
      this.config.clientFolder,
      "shorebird.yaml",
    );
...
`;
      ...
      logger.warn(
        'Execute "cd white_label_app && shorebird init" após setup para gerar app_id real',
      );
      return;
    }
    ...
    try {
      ...
      if (fs.existsSync(whiteLabelShorebirdPath)) {
        ...
        logger.success(
          `shorebird.yaml gerado e copiado para: ${clientShorebirdPath}`,
        );
        ...
        if (content.includes("placeholder-")) {
          logger.warn(
            "shorebird init gerou placeholder - verifique se está logado (shorebird login)",
          );
        } else {
          logger.success(
            "Shorebird configurado com app_id real - OTA updates habilitados!",
          );
        }
      } else {
      ...
      logger.info(
        "Você pode executar manualmente: cd white_label_app && shorebird init",
      );
...
`;
  ...
  async run() {
    try {
      ...
      logger.info(
        "This wizard will create a complete white-label client setup",
      );
      logger.blank();
      ...
      await this.executeStep("create_firebase_project", async () => {
        ...
        this.resourceTracker.trackFirebaseProject(
          this.config.firebaseProjectId,
        );
      });
      ...
      await this.executeStep("save_to_master_firebase", async () => {
        ...
        this.resourceTracker.trackMasterFirebaseEntry(
          this.config.clientCode,
          firebaseClient,
        );
      });
      ...
      await this.executeStep("generate_android_keystore", () =>
        this.generateAndroidKeystore(),
      );
      ...
      await this.executeStep("generate_app_check_instructions", () =>
        this.generateAppCheckSetup(),
      );

      // Step 5.7: Setup Remote Config
      await this.executeStep("setup_remote_config", () =>
      ...
      // Step 8: Commit client config to main
      await this.executeStep("commit_client_config", () =>
        this.commitClientConfig(),
      );

      // Step 9: Generate app store metadata
      await this.executeStep("generate_metadata", () =>
      ...
      // Step 10c: Copy business type assets
      await this.executeStep("copy_business_type_assets", () =>
        this.copyBusinessTypeAssets(),
      );

      // Step 10d: Create Shorebird configuration for OTA updates
      await this.executeStep("create_shorebird_config", () =>
        this.createShorebirdConfig(),
      );
      ...
      if (this.config.adminCredentials) {
        ...
        logger.blank();
        logger.warn(
          "⚠️  IMPORTANTE: Salve essas credenciais em um local seguro AGORA!",
        );
        logger.blank();
        logger.blank();
      }
      ...
      logger.subSection(
        "2️⃣  CONFIGURAÇÃO DO APP CHECK (2 cliques necessários)",
      );
      logger.blank();
      ...
      logger.info(
        '   b) Clique em "Register" (Registrar) sob "Play Integrity"',
      );
      logger.info(
      ...
      logger.blank();
      logger.info(
        "✅ SHA-256 fingerprints já foram registrados automaticamente!",
      );
      logger.info(
      ...
      if (this.checkpointManager && this.checkpointManager.exists()) {
        logger.blank();
        logger.info(
          "💾 Checkpoint saved - you can resume by running the wizard again",
        );
        logger.info(
          "   The wizard will ask if you want to continue from where it left off",
        );
        logger.blank();
      }
