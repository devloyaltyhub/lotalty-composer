...
async function downloadFreshConfigs(clientDir, firebaseProjectId) {
  ...
  if (!firebaseProjectId) {
    console.log(
      "  firebaseProjectId nao encontrado no config.json, pulando download",
    );
    return results;
  }
  ...
  if (!isFirebaseLoggedIn()) {
    console.log(
      '  Firebase CLI nao autenticado. Execute "firebase login" para baixar configs atualizadas.',
    );
  ...
  console.log(
    `  Baixando configs atualizadas do Firebase (${firebaseProjectId})...`,
  );
