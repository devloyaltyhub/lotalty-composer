...
const { findFirstExisting } = require("../../../shared/utils/fs-utils");

function findAndCopy(possiblePaths, dest) {
  const source = findFirstExisting(possiblePaths);
  if (source) {
...
async function copyFirebaseConfigs(clientCode, clientConfig) {
  ...
  residualFiles.forEach((filePath) => {
    if (fs.existsSync(filePath)) {
      ...
      console.log(
        `  Removido arquivo residual: ${path.relative(TARGET_ROOT, filePath)}`,
      );
    }
  });

  const downloaded = await downloadFreshConfigs(
    clientDir,
    clientConfig.firebaseProjectId,
  );

  const androidSource = findAndCopy(
    [
      path.join(clientDir, "android", "google-services.json"),
      path.join(clientDir, "google-services.json"),
    ],
    path.join(TARGET_ROOT, "android", "app", "google-services.json"),
  );
  if (androidSource) {
  ...
  const iosSource = findAndCopy(
    ...
    path.join(TARGET_ROOT, "ios", "Runner", "GoogleService-Info.plist"),
  );
  if (iosSource) {
    const source = downloaded.ios ? "baixado do Firebase" : "cache local";
    console.log(
      `  GoogleService-Info.plist copiado para ios/Runner/ (${source})`,
    );
    copied++;
  } else {
  ...
  if (dartSource) {
    ...
    console.log(
      `  firebase_options.dart copiado para lib/ (de ${relativePath})`,
    );
    copied++;
  } else {
    console.log(
      `  firebase_options.dart não encontrado, tentando gerar automaticamente...`,
    );
    let generated = false;
    ...
    if (projectId) {
      try {
        ...
        execSync(
          `flutterfire configure --project=${projectId} --out=lib/firebase_options.dart --yes`,
          {
            cwd: TARGET_ROOT,
            stdio: "inherit",
            timeout: 180000,
        ...
        if (fs.existsSync(dartDest)) {
          ...
          fs.copyFileSync(
            dartDest,
            path.join(clientLibDir, "firebase_options.dart"),
          );
          console.log(
            "  firebase_options.dart salvo na pasta do cliente para uso futuro",
          );
        }
        generated = true;
        copied++;
        ...
        console.log(
          "  Tentando gerar a partir dos arquivos de configuração existentes...",
        );
      }
    }
  ...
  if (missing.length > 0) {
    ...
    console.log(
      `     firebase apps:sdkconfig android --project <project-id> > android/google-services.json`,
    );
    console.log(
      `     firebase apps:sdkconfig ios --project <project-id> > ios/Runner/GoogleService-Info.plist`,
    );
    console.log("\n  Para regenerar firebase_options.dart:");
    console.log(
      `     cd white_label_app && flutterfire configure --project=<project-id>`,
    );
    console.log(
...
function copyFirebaseJson(clientCode, clientConfig) {
  ...
  if (!fs.existsSync(sourcePath)) {
    ...
    console.log(
      "     FlutterFire CLI pode falhar durante upload de simbolos Crashlytics",
    );
    return;
  }
  ...
  try {
    ...
    if (!firebaseConfig.flutter?.platforms) {
      console.log(
        "  flutter.platforms não encontrado no firebase.json do cliente",
      );
...
function copyShorebirdConfig(clientCode) {
  ...
  if (fs.existsSync(sourcePath)) {
    ...
    if (content.includes("placeholder-")) {
      ...
      console.log(
        '     Execute "cd white_label_app && shorebird init" para gerar app_id real',
      );
    } else {
...
function generateGHAExportOptions(clientConfig) {
  ...
  if (!bundleId) {
    console.log(
      "  ATENÇÃO: bundleId não encontrado no config.json, pulando GHAExportOptions.plist",
    );
    return;
  }
