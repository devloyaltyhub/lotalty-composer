...
function copyFolderRecursiveSync(src, dest, ignorePaths = []) {
  copyFolderRecursive(src, dest, ignorePaths);
}
...
function backupBusinessTypeAssets(assetsDir, businessTypes) {
  ...
  categoriesToClean.forEach((category) => {
    ...
    items.forEach((item) => {
      ...
      if (stat.isDirectory()) {
        const isBusinessTypeDir = businessTypes.some(
          (businessType) => businessType.key === item,
        );
...
function cleanAssetsDir(assetsDir, businessTypes) {
  ...
  try {
    const { backupDir, backedUp } = backupBusinessTypeAssets(
      assetsDir,
      businessTypes,
    );
