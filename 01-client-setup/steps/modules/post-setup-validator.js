...
function validateFirebaseOptions(results, expectedProjectId) {
  ...
  if (!projectId) {
    addError(
      ...
      "Arquivo não encontrado ou projectId não extraído",
      expectedProjectId,
      null,
    );
...
function validateGoogleServiceInfoPlist(
  results,
  expectedProjectId,
  expectedBundleId,
) {
  ...
  if (!projectId) {
    addError(
      ...
      "Arquivo não encontrado ou PROJECT_ID não extraído",
      expectedProjectId,
      null,
    );
  } else if (projectId !== expectedProjectId) {
    ...
    addCheck(
      checks,
      "ios/Runner/GoogleService-Info.plist (PROJECT_ID)",
      projectId,
    );
  }
  ...
  if (bundleId && bundleId !== expectedBundleId) {
    addError(
      errors,
      "ios/Runner/GoogleService-Info.plist",
      "BUNDLE_ID diferente do config.json — build vai falhar. Corrija o bundleId no config.json ou reconfigure o app no Firebase Console",
      expectedBundleId,
      bundleId,
    );
  } else if (bundleId) {
...
function validateGoogleServicesJson(
  ...
  if (!info) {
    addError(
      ...
      "Arquivo não encontrado ou JSON inválido",
      expectedProjectId,
      null,
    );
    return;
  ...
  if (!hasMatchingPackage) {
    addError(
      errors,
      "android/app/google-services.json",
      "Nenhum client com package_name correspondente — build vai falhar. Corrija o bundleId no config.json ou adicione o app no Firebase Console",
      expectedBundleId,
      info.packageNames.join(", "),
    );
  } else {
    addCheck(
      checks,
...
function validateXcodeProject(results, expectedBundleId) {
  ...
  if (!bundleId) {
    addError(
      ...
      "PRODUCT_BUNDLE_IDENTIFIER não encontrado",
      expectedBundleId,
      null,
    );
    ...
    addError(
      errors,
      "ios/Runner.xcodeproj/project.pbxproj",
      "PRODUCT_BUNDLE_IDENTIFIER incorreto",
      expectedBundleId,
      bundleId,
    );
  } else {
