...
const { ensureDir, getScreenshotFiles } = require("./screenshot-fs-utils");
...
class ScreenshotAndroidCopier {
  ...
  copyToAndroidDevice(deviceKey) {
    ...
    const sourceDir = sourceSubdir
      ? path.join(this.mockupsDir, sourceSubdir)
      : this.mockupsDir;
  ...
  copyToAndroid() {
    ...
    if (totalCount === 0) {
      ...
      if (results.tablet?.count > 0)
        parts.push(`${results.tablet.count} tablet`);
